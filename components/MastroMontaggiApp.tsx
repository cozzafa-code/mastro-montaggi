...
const MastroMontaggi = loadDynamic(
  () => import('@/components/MastroMontaggi'),
  { ssr: false }
)

const DS = {
  teal: '#14B8A6', topbar: '#0B1F2A',
  text: '#0B1F2A', textHint: '#94A3B8',
  font: '-apple-system,BlinkMacSystemFont,"Inter",sans-serif',
}

interface Operatore { id: string; nome: string; cognome: string; azienda_id: string }
...
export default function MastroMontaggiApp() {
  ...
  const verificaPin = async (pin: string) => {
    ...
    localStorage.setItem('mastro_montatore', JSON.stringify({
      op: data, exp: Date.now() + 8 * 3600 * 1000
    }))
  }
  ...
  if (loading) return (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'center', minHeight:'100vh', background: DS.topbar }}>
      <div style={{ width:48, height:48, borderRadius:12, background:DS.teal }} />
    </div>
  )
  ...
  return (
    <div style={{ minHeight:'100vh', background:DS.topbar, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', fontFamily:DS.font }}>
      ...
      <div style={{ fontSize:13, color:'#ffffff60', marginBottom:40 }}>PIN operatore</div>
